...
def choose_best_available(ctx: RoundContext) -> Action | None:
    ...
    mouth_target = ctx.mouth_locked_hand or ctx.committed_hand_type
    if mouth_target and ctx.hand_cards:
        ...
        if unconstrained:
            ...
            return PlayCards(
                indices,
                reason=f"mouth target ({mouth_target}) not formable (no discards): "
